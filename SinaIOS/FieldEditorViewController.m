...
@synthesize fieldSections, values, doneButtonTitle, cancelButtonTitle, context, editorIdentifier, delegate;
...
- (void)switchValueDidChange:(NamedSwitch *)switchControl
{
    ...
    if((self.delegate) && ([delegate respondsToSelector:@selector(fieldEditor:changeValueWithType:AndValue:)])){
        
        [delegate fieldEditor:self changeValueWithType:FieldSpecifierTypeSwitch AndValue:(NSString*)[NSNumber numberWithBool:switchControl.on]];
    }

}

-(void) segmentedControlDidChangeValue:(NamedSegmentControl*)segment{
    
    [self.values setObject:[NSNumber numberWithInt:segment.selectedSegmentIndex] forKey:segment.name];
    
    if((self.delegate) && ([delegate respondsToSelector:@selector(fieldEditor:changeValueWithType:AndValue:)])){
        
        [delegate fieldEditor:self changeValueWithType:FieldSpecifierTypeSegment AndValue:(NSString*)[NSNumber numberWithInt:segment.selectedSegmentIndex]];
    }
}
...
//    FieldSectionSpecifier *fSection = [self.fieldSections objectAtIndex:section];
//    if (fSection.showHeader == NO) {
//        [headerView setBackgroundColor:[UIColor clearColor]];
//    }else{
//        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, 3, tableView.bounds.size.width - 10, 18)];
//        label.text = fSection.title;
//        label.font = [UIFont boldSystemFontOfSize:18];
//        
//        label.textColor = parserColor(0x1a8c2c);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (field.type == FieldSpecifierTypeTextView || field.type == FieldSpecifierTypeViewNomal || field.type == FieldSpecifierTypeWebView) {
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ((type == FieldSpecifierTypeEmail) || (type == FieldSpecifierTypePassword) || (type == FieldSpecifierTypeURL) || (type == FieldSpecifierTypeText) || (type == FieldSpecifierTypeNumeric)) {
		CGRect textFieldFrame = CGRectMake(textLabelFrame.origin.x + textLabelFrame.size.width + 10, 11, cell.contentView.frame.size.width - textLabelFrame.size.width - textLabelFrame.origin.x - 20, 25);
		...
		if (value)
			...
			cell.accessoryType = UITableViewCellAccessoryNone;
//        UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapToCheckMark)];
//        [cell.textLabel addGestureRecognizer:tap];
        ...
        NamedSegmentControl *segment = [[NamedSegmentControl alloc] initWithItems:field.segmentTitles];
        CGSize contentSize = cell.contentView.bounds.size;
        CGRect swidthFrame = CGRectMake(0,0, contentSize.width, contentSize.height);
        segment.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
        segment.frame = swidthFrame;
        segment.tag = SEGMENTTAG;
        segment.tintColor = parserColor(0x1a8c2c);
        segment.name = key;
        
        segment.unSelectedItemBackgroundGradientColors=[NSArray arrayWithObjects:parserColor(0xf5eee6),parserColor(0xd3d2bd), nil];
        segment.selectedItemColor = parserColor(0xffffff);
        segment.unselectedItemColor=parserColor(0x1a8c2c);
        
        [segment setSelectedSegmentIndex:field.defaultSegmentChoose];
        
        [segment addTarget:self action:@selector(segmentedControlDidChangeValue:) forControlEvents:UIControlEventValueChanged];
        
        [cell.contentView addSubview:segment];
        
    }else if(type == FieldSpecifierTypeWebView){
        cell.backgroundView = [[UIView alloc] initWithFrame:CGRectZero];
        NamedWebView* webview = [[NamedWebView alloc] initWithFrame:self.view.frame];
        [webview loadHTMLString:field.htmlText baseURL:nil];
        [cell.contentView addSubview:webview];
        
    }else if(type == FieldSpecifierTypeTextView){
        cell.backgroundView = [[UIView alloc] initWithFrame:CGRectZero];
        CGSize contentSize = cell.contentView.bounds.size;
        [self.tableView setSeparatorColor:[UIColor clearColor]];
        NamedTextView* textView = [[NamedTextView alloc] initWithFrame:CGRectMake(0, 0, contentSize.width-5, 320)];
        [textView.layer setBorderWidth: 1.0];
        [textView.layer setCornerRadius:8.0f];
        [textView.layer setMasksToBounds:YES];
        [textView setEditable:NO];
        [textView.layer setBorderColor:[[UIColor grayColor] CGColor]];
        [textView.layer setBorderWidth:1.7f];
        textView.text = field.title;
        cell.frame = CGRectMake(0, 0, contentSize.width, contentSize.height);
        [cell.contentView addSubview:textView];
        
        
    }else if(type == FieldSpecifierTypeViewNomal){
        cell.backgroundView = [[UIView alloc] initWithFrame:CGRectZero];
        [cell.contentView addSubview:field.viewNomal];
    }
    else if(type == FieldSpecifierTypeImageButton){
        //cell.backgroundView = [[UIView alloc] initWithFrame:CGRectZero];
        cell.imageView.image = [UIImage imageNamed:field.imageName];
        //[cell.imageView setFrame:CGRectMake(10, 0, 30, 40)];
        [cell.imageView.layer setCornerRadius:15.0f];
		...
		if ([field.subsections count] == 1) {
			...
			if ([subsection exclusiveSelection]) {
				...
				cell.detailTextLabel.text = section.textExclusiveSelectionDefault;
...
- (void)openSubsection:(FieldSpecifier *)subsectionField
{
	...
	subController.doneButtonTitle = @"Done";
	subController.cancelButtonTitle = @"Cancel";
	subController.isSubSection = YES;
	
	[self.navigationController pushViewController:subController animated:YES];
    //[self presentModalViewController:subController animated:YES];
    UIBarButtonItem *doneButtonItem = [[UIBarButtonItem alloc] initWithTitle:self.doneButtonTitle style:UIBarButtonItemStyleDone target:self action:@selector(done)];
    subController.navigationItem.rightBarButtonItem = doneButtonItem;
...
@implementation NamedSegmentControl
@synthesize name;
- (void)dealloc{
	self.name = nil;
}
...
@synthesize fields, title, description, exclusiveSelection, showHeader,enable, textExclusiveSelectionDefault;
...
@synthesize type, subsections, key, title, defaultValue, placeholder, shouldDisplayDisclosureIndicator,segmentTitles,defaultSegmentChoose, htmlText, imageName,disable,viewNomal;
...
+ (FieldSpecifier *)segmentFieldWithKey:(NSString *)k segments:(NSArray*)titles defaultSegment:(NSInteger)segmentChoose{
    ...
    field.segmentTitles = titles;
    field.defaultSegmentChoose = segmentChoose;
    
    return field;
}

+ (FieldSpecifier *)webViewFieldWithKey:(NSString *)k htmlData:(NSString*)html{
     FieldSpecifier *field = [FieldSpecifier fieldWithType:FieldSpecifierTypeWebView key:k];
    field.htmlText = html;
    
    return field;
}
+ (FieldSpecifier *)textViewFieldWithKey:(NSString *)k content:(NSString*)text{
     FieldSpecifier *field = [FieldSpecifier fieldWithType:FieldSpecifierTypeTextView key:k];
    field.title = text;
    return field;
}
+ (FieldSpecifier *)viewNomalFieldWithKey:(NSString *)k viewNomal:(UIView*)view{
     FieldSpecifier *field = [FieldSpecifier fieldWithType:FieldSpecifierTypeViewNomal key:k];
    field.viewNomal = view;
    return field;
}

+ (FieldSpecifier *)imageButtonFieldWithKey:(NSString *)k imageName: (NSString*)image textTitle:(NSString*)title{
    FieldSpecifier *field = [FieldSpecifier fieldWithType:FieldSpecifierTypeImageButton key:k];
