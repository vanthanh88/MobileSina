...
-(void) awakeFromNib
{
    ...
    self.contentSize = CGSizeMake(self.frame.size.width, 44);
    self.alwaysBounceHorizontal = NO;
    self.alwaysBounceVertical = NO;
    [self reloadData];
}
...
-(void) reloadData
{
    ...
    UIFont *buttonFont = [UIFont fontWithName:@"Helvetica-Bold" size:18.0f];
     
    int buttonPadding = self.frame.size.width/3-40;
    ...
    {
        if([[dataSource horizMenu:self titleForItemAtIndex:i] isKindOfClass:[NSString class]]){
        ...
        [self addSubview:customButton];
            ...
            UIImage *img = (UIImage*)[dataSource horizMenu:self titleForItemAtIndex:i];
            UIButton *customButton = [UIButton buttonWithType:UIButtonTypeCustom];
            ...
            int buttonWidth = img.size.width;
            
            customButton.frame = CGRectMake(xPos, 12, buttonWidth , 18);
            xPos += buttonWidth;
            xPos += buttonPadding-20;
            [self addSubview:customButton];
...
-(void) buttonTapped:(id) sender
{
    ...
    for(int i = 0; i < self.itemCount; i++)
    {
        ...
        if(i + kButtonBaseTag == button.tag){
            //NSLog(@"Button tapped! %d",i+kButtonBaseTag);
            thisButton.selected = YES;
